...
from src.adapters.action_specs import launch_macro_update_ignore
...
class GenshinPCBetterGIProfile(AutomationProfilePort):
    @property
    def profile_name(self) -> str:
        return "genshin_pc_bettergi"
    ...
    def build_plan(self, request: RunRequest) -> WorkflowPlan:
        ...
        if bool(override.get("use_text_signal_wait", False)):
            wait_params.update(
                {
                    "scene_ready_text_any": ["点击进入", "进入游戏", "开始游戏"],
        ...
        steps = [
            WorkflowStep(name="start_pc_game", kind="game.launch", params={**shared_genshin}),
            WorkflowStep(name="pc_skip_queue", kind="game.queue.enter", params={"strategy": "none", **shared_genshin}),
            WorkflowStep(name="wait_pc_game_ready", kind="game.wait.scene", params=wait_params),
            WorkflowStep(
            ...
            nodes=[
                ...
                StateNode(
                    ...
                    action=ActionIntent(
                        ...
                        params={"strategy": "none", **shared_genshin},
                        controller_id="genshin_controller",
                        required_context="genshin_window",
                    ),
                    controller_id="genshin_controller",
